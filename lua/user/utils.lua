...
--- 设置按键映射，添加自定义opts
--- @param custom_opts 自定义opts，一般是{ description: ""}
M.set_keymap = function(modes, lhs, rhs, opts, custom_opts)
	local new_opts = vim.tbl_extend("force", opts, custom_opts)

	vim.keymap.set(modes, lhs, rhs, new_opts)
end

M.notify_error = function(title, msg)
